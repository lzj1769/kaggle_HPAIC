...
class BatchBalanceImageDataGenerator(Sequence):
    ...
    def __len__(self):
        return int(math.floor(len(self.indexes_0) / self.batch_size))

    def __getitem__(self, index):
        # Generate indexes of the batch
        batch_size = int(self.batch_size / 2)

        indexes_0 = self.indexes_0[index * batch_size: (index + 1) * batch_size]
        indexes_1 = self.indexes_1[index * batch_size: (index + 1) * batch_size]
